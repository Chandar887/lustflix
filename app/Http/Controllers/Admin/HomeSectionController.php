...
class HomeSectionController extends MainAdminController
{
    ...
    { 
        ...
        $short_film_list = Sports::orderBy('id','DESC')->get();
        $regional_list = DB::table('channels_list')->orderBy('id','DESC')->get();
        
        // $regional_list = LiveTv::orderBy('id','DESC')->get();
        return view('admin.pages.home_section',compact('page_title','language_list','movies_list','series_list','home_settings','short_film_list','regional_list'));
    }	 
    ...
    {  
        ...
        if(isset($inputs['section6_short_films']))
        {
        $settings->section6_short_films = implode(',', $inputs['section6_short_films']);
        }
        if(isset($inputs['section7_regional']))
        {
        $settings->section7_regional = implode(',', $inputs['section7_regional']);
        }
